...
const Match = ({home, away, score1, score2, dateStart, homeId, awayId}) => {
    ...
    const Manager = squad===undefined?[]:squad.filter(item=>item.role==='COACH')
    const Assistant = squad===undefined?[]:squad.filter(item=>item.role==='ASSISTANT_COACH')
    const Pemain = squad===undefined?[]:squad.filter(item=>item.role==='PLAYER')
    ...
    return (
        ...
        <div className='grid grid-cols-3 py-1 text-center px-4 font-bold'>
            <div className='text-left' >
                <h1>{home===null?'TBD':home}</h1>
            </div>
            <div>
                <h1>{score1===null?'?':score1}  :  {score2===null?'?':score2}</h1>
            </div>
            <div className='text-right'>
                <h1>{away===null?'TBD':away}</h1>
            </div>
        </div>
        <div className='grid grid-cols-2 cursor-pointer'>
            <div className={show===false?'pl-8':'hidden'}>
                <div className={home===null?'hidden':'block'} onClick={()=>clickHome()}>
                    <BsChevronDown className='h-6 w-6'/>
                </div>
            </div>
            <div className={show===false?'pr-8':'hidden'}>
                <div className={away===null?'hidden':'block'} onClick={()=>clickAway()}>
